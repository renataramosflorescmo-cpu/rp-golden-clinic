...
const AboutSection = () => {
  ...
  useEffect(() => {
    supabase.from("site_about").select("*").limit(1).single().then(({ data }) => { if (data) setAbout(data); });
  }, []);
  return (
        ...
        <div className="grid lg:grid-cols-2 gap-8 items-center">
          ...
          >
            ...
            {(about?.description || "").split("\n\n").map((p: string, i: number) => (
              <p key={i} className="font-body text-muted-foreground leading-relaxed mb-4 font-light">{p || "Proprietária e fundadora da RP Golden Clinic. Médica CRM 160891, Especialista em Dermatologia em São Paulo."}</p>
            ))}
          ...
          >
            <div className="relative">
              ...
              />
              {/* Label overlapping bottom of image to cover crop */}
              <motion.div
                initial={{ opacity: 0, y: 10 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ duration: 0.6, delay: 0.5 }}
                className="absolute z-20 bottom-[22%] inset-x-0 mx-auto w-fit bg-primary/90 px-10 py-3 rounded-full whitespace-nowrap"
              >
                <p className="font-body text-xs tracking-[0.25em] uppercase text-primary-foreground text-center">
                  {about?.badge_text || "Fundadora da RP Golden Clinic"}
                </p>
              </motion.div>
            </div>
          </motion.div>
